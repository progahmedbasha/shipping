		...
		<div class="col-xl-12">
			<div class="card">
				<div class="card-header pb-0">
					<div class="d-flex justify-content-between">  	<!-- <input type="number" style="width:55%;" value="{{ $orders->id+1 }}" disabled> -->
						<h4 class="card-title mg-b-0">رقم الفاتورة :
							@if ($orders && $orders->count() > 0)
							
							<input type="number" style="width:55%;" value="{{ $orders->id+1 }}" disabled>							
							@endif
						</h4>
...
<!-- 					<p class="tx-12 tx-gray-500 mb-2">Example of Valex Simple Table. <a href="">Learn more</a></p>
 -->				</div>
				...
				{{-- START SUBMIT FORM FOR ORDER TABLE  --}}
					<form action="{{ route('orders.store') }}" method="post" @if ($orderDetailes->count() < 1)
						hidden
					@endif>
						@csrf
						<div class="row">
...
<!-- 							@if ($orders && $orders->count() > 0)
							<div class="col-md-3">
								<div class="form-group">
									<label for="">رقم الفاتورة</label>
										...
										<input type="number" style="width:40%;" value="{{ $orders->id+1 }}" disabled>
							...
							<div class="col-md-3">
								<div class="form-group">
									...
									<button class="btn btn-primary btn-lg" style="margin-top:10%;">
										اضافة الاوردر
									</button>
								</div>
							</div>
				...
				{{-- END SUBMIT FORM FOR ORDER TABLE  --}}
					...
					<div class="table-responsive">
						@if ($orderDetailes && $orderDetailes->count() > 0)
							...
							<!-- 						
													<input type="number" class="price{{ $item->id }}" name="price{{ $item->id }}" style="width: 45px;" id="price{{ $item->id }}" value="{{ $item->shipping_price }}">
													<button row_id="{{ $item->id }}" class="btn btn-primary btn-sm" class="change_price btn btn-success">اضافة</button> -->

														<input type="number" class="price{{ $item->id }}" name="price{{ $item->id }}" style="width: 100%" id="price{{ $item->id }}" value="{{ $item->shipping_price }}">
													
													<button row_id="{{ $item->id }}" style="background-color:#4266f5;" class="change_price btn btn-success">اضافة</button>
											...
											<td>						{{-- CHANGE STATUS --}}
												<form action="" class="status">										
													<select style="height: 28px; width:65px; font-size:13px;" name="status_id{{ $item->id }}" id="package_status{{ $item->id }}" class="st_id{{ $item->id }} form-control">
